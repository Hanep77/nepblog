...
@section('container')
    <h2 class="text-2xl font-semibold mb-5">Edit Post</h2>
    ...
    <form action="/dashboard/posts/update/{{ $post->id }}" method="post" enctype="multipart/form-data">
        ...
        <div class="w-96">
            <input type="text" name="title"
                ...
                id="title" value="{{ old('title') ? old('title') : $post->title }}" required>
            @error('title')
        ...
        <div class="w-96 mt-3">
            <input type="text" name="slug"
                ...
                id="slug" value="{{ old('slug') ? old('slug') : $post->slug }}" required>
            @error('slug')
                <p class="text-red-600 italic">{{ $message }}</p>
            @enderror
        </div>
            ...
            <select name="category_id" class="w-full h-8 rounded bg-slate-700 bg-opacity-30 border border-slate-800 pr-2"
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}"
                        @if (old('category_id') == $category->id) selected @elseif(!old('category_id') && $post->category_id == $category->id) selected @endif
        ...
        <div class="w-96 mt-3">
            ...
            <img src="{{ asset('storage/' . $post->image) }}" @class(['w-60 mt-2', 'hidden' => !$post->image]) id="preview">
            @error('image')
                <p class="text-red-600 italic">{{ $message }}</p>
            @enderror
        </div>
        <div class="mt-3 w-[768px] text-lg text-white">
            <input id="x" type="hidden" name="content"
                value="{{ old('content') ? old('content') : $post->content }}" required>
